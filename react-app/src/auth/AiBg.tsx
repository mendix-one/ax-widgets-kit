...
/**
 * Animated SVG background themed around aPlanner's manufacturing domain.
 *
 * Elements:
 * - Silicon wafer with die grid (slowly rotating) — semiconductor focus
 * - Manufacturing process flow pipeline with step nodes
 * - Yield trend line — key performance metric
 * - KPI bar chart — real-time analytics
 * - Fab equipment silhouettes — factory context
 * - 7-pillar ecosystem hexagons — Amoza platform vision
 * - Data-stream particles along process paths — AI intelligence
 *
 * All animations use CSS keyframes + SMIL for GPU-accelerated performance.
 */
export const AiBg = memo(function AiBg() {
  return (
    <svg
    ...
    >
      ...
      {/* Scan line sweeping across wafer */}
      ...
      {/* Input path — deposition → lithography → etch → wafer center */}
      <path
        className="conn"
        ...
        strokeWidth="0.8"
      />
      {/* Output path — wafer center → test → packaging → ship */}
      <path
      ...
      {/* Scenario simulation branches (what-if paths) */}
      <path
      ...
      <g>
        ...
        {/* Reverse data feedback particle */}
      ...
      {/* ── 7-Pillar ecosystem hexagons (top-right) ── */}
